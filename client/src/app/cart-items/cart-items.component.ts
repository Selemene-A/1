import { Component, OnInit , PLATFORM_ID, Inject, ɵConsole} from '@angular/core';
declare var $: any ;
...
export class CartItemsComponent implements OnInit {
          ...
          this.setCalculation();
           ...
           this.detach();
            ...
            this.cartManagementService.getCustomerCart(this.token).subscribe(data => {
              console.log(data)
              if(data.success === true && data.cart){
                this.itemsInCart = data.cart.cart;
                this.setCalculation();
                if ( isPlatformBrowser(this.platformId) ) {
  ...
  removeItem(cart) {
    ...
    if (this.itemsInCart) {
       this.itemsInCart = this.itemsInCart.filter ( item =>  item._id !== cart._id );
       if (!usersModule.isRegisteredUser(data)) {
