      ...
      if (isPlatformBrowser(this.platformId)) {
        guestCart =  localStorage.getItem('guest_cart');
           ...
           /************** passwords don't match ********/
            ...
            this.signUpService.validateUserPassword(object).subscribe(data => {
              let user = data.user;
              if(data.success === false) {
                this.detach();
                this.wrongEmailAndOrPassword();            
              }else {  
                console.log(data.user,'sdsadasdsadsadsadsadasd')
                // return;              
...
/******************** ends  *************/
  ...
  isAdmin(){
    ...
    if(data) {
      if(data.session === 'loggedIn' && data.mode === 'registered'&& data.email === 'larastar721@gmail.com') {
           this.admin = true;
       }
    }
  }
